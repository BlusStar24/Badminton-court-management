    ...
    $.getJSON("/AdminQuanLy/GetBookings", {
        ...
        for (const key in grouped) {
            ...
            while (i < list.length) {
                ...
                const cell = document.querySelector(selector);
                cell.dataset.hour = merged.start_time.toFixed(1); // BỔ SUNG DÒNG NÀY
...
window.onload = () => {
    ...
    $(document).on('change', '.day-checkbox, .hour-checkbox', () => {
        filterBookingTableNew();
        setTimeout(removeCellsHiddenByRowspan, 10); // Delay nhẹ để đảm bảo DOM cập nhật xong
    });
